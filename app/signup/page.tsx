...
export default function SignupPage() {
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			setServerMessage("Application submitted! We will get in touch soon.");
		...
		<div className="min-h-screen bg-linear-to-br from-slate-950 via-purple-950 to-black text-slate-200">
			<div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 py-16 md:flex-row">
				<section className="flex-1 space-y-6 rounded-3xl border border-purple-700/40 bg-black/40 p-8 shadow-[0_0_40px_rgba(168,85,247,0.35)] backdrop-blur">
					<p className="text-sm uppercase tracking-[0.3em] text-purple-400">NovaForge Labs</p>
					<h1 className="text-4xl font-semibold text-white md:text-5xl">
						Join the <span className="text-purple-400">Founders Guild</span>
					</h1>
					<p className="text-lg text-slate-300">
						Submit your startup or game concept to unlock mentorship, funding, and
						a high-energy community of dreamers building the next generation of
				...
				<section className="flex-1">
					...
					>
						<h2 className="text-2xl font-semibold text-white">Signup Mission Log</h2>
						<p className="mt-1 text-sm text-slate-300">
							Complete the intel below so our scouting squad can review your build.
						</p>
						...
						<div className="mt-6 grid gap-5 md:grid-cols-2">
							...
							<label className="flex flex-col gap-2">
								...
								<input
									...
									onChange={(event) => handleChange("linkedInProfile", event.target.value)}
								/>
							</label>

							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Startup Name</span>
								<input
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
							...
							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Product / Game Title *</span>
								<input
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									placeholder="Starborne Chronicles"
									value={form.productTitle}
									onChange={(event) => handleChange("productTitle", event.target.value)}
									required
								/>
							</label>

							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Website / Portfolio</span>
								<input
									type="url"
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									placeholder="https://novaforge.gg"
									value={form.websiteOrPortfolio}
									onChange={(event) => handleChange("websiteOrPortfolio", event.target.value)}
								/>
							</label>

							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Origin City</span>
								<select
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									value={form.startupOriginCity}
									onChange={(event) => handleChange("startupOriginCity", event.target.value)}
								>
							...
							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Team Members *</span>
								<input
									type="number"
									min={1}
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									value={form.teamMembersCount}
									onChange={(event) => handleChange("teamMembersCount", event.target.value)}
									required
								/>
							</label>
							...
							<label className="flex flex-col gap-2">
								...
								<select
									...
									onChange={(event) => handleChange("currentStage", event.target.value)}
								>
							...
							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Vertical Focus</span>
								<select
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									value={form.vertical}
								...
								>
									{verticalOptions.map((vertical) => (
										<option key={vertical} value={vertical} className="bg-slate-900">
											{vertical}
										</option>
									))}
								</select>
							...
							<label className="md:col-span-2 flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Biggest Challenges</span>
								<textarea
									...
									onChange={(event) => handleChange("challengesFaced", event.target.value)}
								/>
							</label>

							<label className="md:col-span-2 flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Problem Statement</span>
								<textarea
									className="min-h-[110px] rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
							...
							<label className="md:col-span-2 flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Solution Summary</span>
								<textarea
									className="min-h-[110px] rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									placeholder="How does your build change the game?"
									value={form.solutionSummary}
									onChange={(event) => handleChange("solutionSummary", event.target.value)}
								/>
							</label>

							<label className="md:col-span-2 flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Previous Incubation Experience</span>
								<textarea
									...
									onChange={(event) => handleChange("previousIncubationExperience", event.target.value)}
								/>
							</label>

							<label className="flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Revenue (Last 12 Months) *</span>
								<input
									type="number"
									min={0}
									className="rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									value={form.revenueLast12Months}
									onChange={(event) => handleChange("revenueLast12Months", event.target.value)}
									required
								/>
							</label>

							<label className="md:col-span-2 flex flex-col gap-2">
								<span className="text-sm font-medium text-slate-200">Motivation To Apply</span>
								<textarea
									className="min-h-[110px] rounded-xl border border-purple-500/30 bg-black/60 px-4 py-3 text-sm text-white outline-none transition focus:border-purple-400 focus:ring-2 focus:ring-purple-500/40"
									placeholder="Why are you ready to enter the arena now?"
									value={form.motivationToApply}
									onChange={(event) => handleChange("motivationToApply", event.target.value)}
								/>
							</label>
						</div>
						...
						<div className="mt-8 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
							...
							<p className="text-xs text-slate-400">
								By submitting you certify that your data is accurate and you&apos;re ready for lift-off.
							</p>
						</div>
					</form>
				</section>
